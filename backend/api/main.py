...
# Import REAL ADK agents instead of scripted functions
from run_adk import run_healio_adk_pipeline

# Import Firebase real-time managers
...
async def analyze_patient(
    ...
    try:
        ...
        logger.info(f"[ANALYZE] REAL ADK agents starting: {text_input[:50]}...")
        ...
        logger.info(f"[ANALYZE] REAL agents complete: {patient_id} → {response['triage_color']}")
        ...
        logger.error(f"Error in REAL ADK analyze: {str(e)}")
...
async def analyze_with_multimodal(
    ...
    try:
        ...
        # Run through multimodal pipeline
        ...
        patient_id = queue_manager.add_patient_to_queue(patient_card)
        
        # Record surveillance
        if background_tasks:
            background_tasks.add_task(
                surveillance.record_surveillance_data,
                handoff_payload.get("surveillance_data", {})
            )
...
async def startup_event():
    ...
    # Start queue listener with thread-safe callback
    ...
    # Start cluster listener with thread-safe callback
    ...
    logger.info("✅ Real-time listeners initialized")
